...
export default function GamePage() {
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden" style={{ background: 'var(--color-bg-dark)' }}>
        ...
        <div className="px-3 py-2 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {isDrawer && currentWord ? (
              <div className="px-2 py-1 rounded-lg flex items-center gap-1" style={{ background: 'rgba(255,107,53,0.15)' }}>
                <Pen className="w-3 h-3" style={{ color: 'var(--color-primary)' }} />
                <span className="text-xs font-bold hidden sm:inline" style={{ color: 'var(--color-primary)' }}>你的词：</span>
                <span className="text-sm sm:text-lg font-bold text-white font-display">{currentWord}</span>
              </div>
            ) : !isDrawer ? (
              <div className="px-2 py-1 rounded-lg flex items-center gap-1" style={{ background: 'rgba(255,217,61,0.1)' }}>
                <Eye className="w-3 h-3" style={{ color: 'var(--color-accent-yellow)' }} />
                <span className="text-xs hidden sm:inline" style={{ color: 'var(--color-accent-yellow)' }}>提示：</span>
                <span className="text-sm sm:text-lg font-bold text-white font-display tracking-widest">{currentHint || '___'}</span>
              </div>
            ) : null}
      ...
      <main className="flex-1 flex min-h-0 w-full p-2 gap-2 overflow-hidden">
        ...
        <div className="hidden md:flex w-56 shrink-0 flex-col gap-2 min-h-0">
      ...
      {!isDrawer && !hasGuessed && room.status === 'playing' && (
        <div className="md:hidden shrink-0 p-2 bg-black/40 backdrop-blur-md border-t border-white/10">
          <form onSubmit={handleGuess} className="flex gap-2">
            <input
              ...
              onChange={(e) => setGuessInput(e.target.value)}
              placeholder="输入你的猜测..."
              className="flex-1 px-3 py-2 bg-white/10 border border-white/10 rounded-lg text-white text-sm placeholder-white/30 focus:outline-none focus:ring-1 focus:ring-[var(--color-primary)]/30"
              maxLength={20}
              autoComplete="off"
            />
            <button type="submit" className="px-4 py-2 rounded-lg text-white font-bold text-sm transition-all active:scale-90" style={{ background: 'var(--color-primary)' }}>
              猜
            </button>
          </form>
        </div>
